...
public class AutoHbmService extends Service {
    ...
    // klee: a max fényerő a kernel max_brightness node-jából jön (MTK skála
    // eltér a Qualcomm 4000-től — tipikusan 255 vagy 2047), lustán cache-elve.
    private static int sMaxBrightness = -1;

    private static int maxBrightness() {
        if (sMaxBrightness <= 0) sMaxBrightness = Constants.getMaxBrightness();
    ...
    private boolean isCurrentlyEnabled() {
        ...
        return Integer.parseInt(fileValue) == maxBrightness();
    }
            ...
            if (lux > luxThreshold) {
                if (!mCrossedThreshold) {
                    ...
                    if (currentTime - mCrossedThresholdTime >= timeToEnableHbm * 1000 && (!mAutoHbmActive || !isCurrentlyEnabled()) && !keyguardShowing) {
                        ...
                        setBrightnessDirectly(maxBrightness());
