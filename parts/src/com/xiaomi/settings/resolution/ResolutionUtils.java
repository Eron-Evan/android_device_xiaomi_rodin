...
package com.xiaomi.settings.resolution;
 ...
 *
 * Device baseline — POCO X8 Pro (klee / MT6899, Dimensity 8500 Ultra):
 *   Physical AMOLED panel : 1268 × 2756 px  (1.5K, 6.59", 460 ppi)
 *   Fallback display density: 480 dpi (verify with `adb shell wm density`)
 *
 * Note: the constants below are FALLBACKS only. At runtime the real values are
 * queried from Display.Mode / IWindowManager; the constants are used when a
 * forced size is already applied at query time or the query fails.
 */
public final class ResolutionUtils {
    ...
    // -------------------------------------------------------------------------
    // klee (MT6899 / POCO X8 Pro) hard-coded panel baseline (fallback).
    // Physical 1.5K AMOLED panel: 1268 × 2756 @ 460 ppi; HyperOS/Lineage
    // default density 480 dpi. Verify on-device: `adb shell wm size` / `wm density`.
    // -------------------------------------------------------------------------
    private static final int KLEE_STOCK_WIDTH   = 1268;
    private static final int KLEE_STOCK_HEIGHT  = 2756;
    private static final int KLEE_STOCK_DENSITY = 480;
    ...
    private void initializeStockBaselines() {
        try {
            ...
            if (pw >= KLEE_STOCK_WIDTH && ph >= KLEE_STOCK_HEIGHT) {
                mStockWidth  = pw;
                mStockHeight = ph;
            } else {
                mStockWidth  = KLEE_STOCK_WIDTH;
                mStockHeight = KLEE_STOCK_HEIGHT;
            }
        } catch (Exception e) {
            mStockWidth  = KLEE_STOCK_WIDTH;
            mStockHeight = KLEE_STOCK_HEIGHT;
        ...
        try {
            ...
            mInitialDensity = (d >= 280 && d <= 640) ? d : KLEE_STOCK_DENSITY;
        } catch (RemoteException e) {
            mInitialDensity = KLEE_STOCK_DENSITY;
        }
    }
